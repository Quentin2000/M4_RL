...
def action_rate_elevation(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:

    """Penalize the rate of change of the actions using L2-kernel."""
    return abs(env.action_manager.action[:, 2] - env.action_manager.prev_action[:, 2])
...
def reversed_m4(env: ManagerBasedRLEnv, std: float, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    reward = relu(asset.data.projected_gravity_b[:, 2]/std)
