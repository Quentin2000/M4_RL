...
import omni.isaac.lab_tasks.manager_based.classic.m4_traversability.mdp as mdp
...
# def display_depth_data_live(depth_data):
#     plt.clf()
#     #fig, ax = plt.subplots()

#     def update_fig(*args):
#         # Convert the tensor to numpy array and replace 'inf' with a maximum depth value for better visualization
...
#         img.set_array(depth_data_np)
#         return img,

#     # Initial conversion and display
#     depth_data_np = depth_data.cpu().numpy()
#     max_value = np.nanmax(depth_data_np[np.isfinite(depth_data_np)])
#     depth_data_np[depth_data_np == float('inf')] = max_value + 1
...
def camera_depth(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("camera")) -> torch.Tensor:
    ...
    depth[torch.isinf(depth)] = 100
    # print("Depth: ", depth)
    
    # Visualization
    display_depth_data_live(depth.squeeze())
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    camera = CameraCfg(
        ...
        offset=CameraCfg.OffsetCfg(pos=(0.24099, 0.0, -0.02), rot=(0.5, -0.5, 0.5, -0.5), convention="ros"),
    )
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityXCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(0.0, 0.5), ang_vel_z=(-0.3, 0.3)
        ),
    )
...
class ActionsCfg:
    ...
    wheel_vel = mdp.JointVelocityActionCfg(asset_name="robot", joint_names=[".*wheel_joint"], scale=5.0, use_default_offset=False, debug_vis=True)
...
class RewardsCfg:
    ...
    track_ang_vel_z = RewTerm(
        func=mdp.track_ang_vel_z_m4, weight=-10.0, params={"command_name": "base_velocity"}
    )
    ...
    rolling_not_crawling = RewTerm(func=mdp.rolling_not_crawling, weight=-10)
...
class TerminationsCfg:
    ...
    # func=mdp.lin_speed_limit_reached_full,
    # params={"threshold": 2.0}
    # )
    # ang_speed_limit_reached_full = DoneTerm(
    # func=mdp.ang_speed_limit_reached_full,
    # params={"threshold": 2.0}
    # )
...
@configclass
class M4TraversabilityEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.decimation = 20 # actions every 0.1 sec (self.sim.dt * self.decimation)
